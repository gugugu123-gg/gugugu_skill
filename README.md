...
这是我自用的一组 Codex / OpenAI skill，主要围绕中文写作排版、Obsidian/self vault 笔记整理、日记录沉淀、双向链接维护和长期自我模型更新。

这些 skill 的共同原则是：少发明、多保留；少润色、多显形；尽量把原始材料中已经存在的结构、判断、证据和线索整理出来，而不是把它们改写成泛泛的 AI 文案。
...
把需要的 skill 目录复制到 Codex 的个人 skills 目录下即可。每个 skill 目录都以 `SKILL.md` 为入口，并可包含：

- `references/`：规则、模板和判断标准
- `agents/openai.yaml`：在 OpenAI / Codex 界面中的展示名、简介和默认 prompt

可以整体复制本仓库中的所有目录，也可以只复制其中几个常用目录。
...
- 文件边界清晰：排版、沉淀、日记录、双链、自我模型各自负责不同阶段。
